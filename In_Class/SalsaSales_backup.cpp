				...
				if (largest <= arr[pos]) // compare the current largest value with the current value in the array
					{
						...
						larg_list.push_back(largest);
					}
			}
		if (larg_list.size() > 1)
			{
				cout << "\nthere was more than one largest salsa type sold\n";
				for (size_t type = 0; type < larg_list.size(); type++)
					{
						cout << larg_list[type] << " ";
					}
			}
				...
				if (smallest >= arr[pos]) // the only difference between the two functions, if the smallest is larger, then
					{
						...
						smal_list.push_back(smallest);
					}
			}
		if (smal_list.size() > 1)
			{
				cout << "\nthere was more than one smallest salsa type sold\n";
				for (size_t type = 0; type < smal_list.size(); type++)
					{
						cout << smal_list[type] << " ";
					}
			}
